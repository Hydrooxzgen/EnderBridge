...
首次运行（或 `config.example.json` 中 `is_first_run = True`）会自动启动**图形化WebUI配置向导**, 浏览器访问 `http://127.0.0.1:18888` 完成配置：

On first run (or when `is_first_run = True` in `config.example.json`), the **web setup wizard** starts automatically. Open `http://127.0.0.1:18888` in your browser to configure:
...
保存后自动生成配置文件（旧文件备份为 `.bak`）, **服务器自动启动**, 无需手动重启。

After saving, the config files are generated (old files backed up as `.bak`). **The server starts automatically** — no manual restart needed.
...
| `python main.py --load-without-config` | 忽略配置文件, 直接启动服务器（仅用于调试）/ Start the server without loading config (debug only) |
| `python main.py --help` | 显示帮助信息 / Show help information |
| `python main.py -h` | 相当于`--help`参数 / Equivalent to the `--help` argument |
...
| `python main.py --version` | 查看当前EnderBridge版本 / View the current version |
| `python main.py -v` | 相当于`--version`参数 / Equivalent to the `--version` argument |
...
3. 覆盖代码文件, **保留** 配置文件及其备份、`resources/`、`structures/`、`logs/`、`.git/` 和自定义文件
4. 完成后提示重启：`python main.py`

> 校验失败或压缩包损坏时不会改动任何现有文件。若新版配置模板结构变化, 启动异常时可运行 `python main.py --reset-all` 重置配置。
...
2. 打包全部代码与模板（`main.py` / `lib/` / `mod/` / `webui/` / `wiki/` / `config.example.json` / `permission.example.json` 等）
...
├── config.example.json        # 配置模板（含 is_first_run 标记）/ Config template
...
检查 `config.example.json` 中 `is_first_run` 是否为 `True`, 或运行 `python main.py --reset-all` 复位后重启（向导仅在首次运行时启动）。日常管理请使用 Web 管理界面：启动服务器后访问 `http://127.0.0.1:18888`。
Check `is_first_run` in `config.example.json`, or run `python main.py --reset-all` to reset and restart (the wizard only starts on first run). For daily management, use the Web console at `http://127.0.0.1:18888` after starting the server.

**Q：向导保存报「模板匹配失败」？/ Wizard reports "template match failed"?**
确保 `config.example.json` 未被手动改动关键字段格式（向导按模板结构读取并填充用户输入）。
Ensure `config.example.json` keeps its original format (the wizard reads the template structure and fills in user input).
...
*You should try our sister project: [ModLoader-WS-For-MCBE](https://github.com/StarAwA117/ModLoader-WS-For-MCBE)*

---

***EnderBridge · Minecraft Bedrock 服务器管理框架***
<br>
***Powerd by Hydrooxzgen(Hydrooxygen)***
